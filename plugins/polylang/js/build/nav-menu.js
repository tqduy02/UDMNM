...
const pllNavMenu = {
  ...
  printMetabox: {
    ...
    handleEvent: (event) => {
      ...
      if (
        !metabox.querySelectorAll('input[value="#pll_switcher"][type=text]')
          .length
      ) {
      ...
      ids.forEach((optionName) => {
        ...
        if (
          "hide_current" === optionName &&
          isValDefined &&
          1 === pll_data.val[itemId].dropdown
        ) {
        ...
        if (
          (isValDefined && 1 === pll_data.val[itemId][optionName]) ||
          (!isValDefined && "show_names" === optionName)
        ) {
  ...
  ensureContent: {
    ...
    handleEvent: (event) => {
      ...
      document.getElementById(
        `edit-menu-item-show_${otherType}-${id}`,
      ).checked = true;
    },
  },
  ...
  showHideRows: {
    ...
    handleEvent: (event) => {
      ...
      const hideCb = document.getElementById(
        `edit-menu-item-hide_current-${matches[1]}`,
      );
