...
jQuery(function ($) {
  ...
  function addLanguage(language) {
    ...
    var languageValueHtml = $("<td />")
      .text(language.text)
      .prepend(language.flagUrl); // phpcs:ignore WordPressVIPMinimum.JS.HTMLExecutingFunctions.prepend
    ...
    var languageLineHtml = $("<tr />")
      .prepend(languageTrashIconHtml)
      .prepend(languageValueHtml); // phpcs:ignore WordPressVIPMinimum.JS.HTMLExecutingFunctions.prepend
  ...
  $("#add-language").on("click", function (event) {
    hideError();
    var selectedOption =
      event.currentTarget.form.lang_list.options[
        event.currentTarget.form.lang_list.selectedIndex
      ];
    if (
      "" !== selectedOption.value &&
      !languagesMap.has(selectedOption.value)
    ) {
      addLanguage({
  ...
  addLanguageForm.on("submit", function (event) {
    ...
    var isLanguagesAlreadyDefined =
      definedLanguagesListTable.children().length > 0;
  ...
  dialog.dialog({
    ...
    open: function (event, ui) {
      ...
      $(this)
        ...
        .text($("#lang_list").children(":selected").first().text());
