/******/ "use strict"; // ./js/src/lib/confirmation-modal.js
...
const initializeConfirmationModal = () => {
  ...
  const dialogContainer = jQuery("<div/>", {
    ...
    __(
      "Are you sure you want to change the language of the current content?",
      "polylang",
    ),
  );
  ...
  const dialogResult = new Promise((confirm, cancel) => {
    const confirmDialog = (what) => {
      ...
      switch (
        ...
        case "yes":
          ...
          languagesList.data(
            "old-value",
            languagesList.children(":selected").first().val(),
          );
          confirm();
          break;
        case "no":
    ...
    if (jQuery.ui.version >= "1.12.0") {
      Object.assign(dialogOptions, {
        classes: { "ui-dialog": "pll-confirmation-modal" },
      });
    } else {
  ...
  jQuery(".tr_lang").each(function () {
    ...
    jQuery(this).autocomplete({
      ...
      source:
        ...
        "&post_language=" +
        jQuery(".post_lang_choice").val() +
        "&translation_language=" +
        tr_lang +
        "&post_type=" +
        jQuery("#post_type").val() +
        "&_pll_nonce=" +
        jQuery("#_pll_nonce").val(),
...
const filterPathMiddleware = (options, filteredRoutes, filter) => {
  ...
  return Object.values(filteredRoutes).find((path) => cleanPath === path)
    ? filter(options)
    : options;
};

/* harmony default export */ const filter_path_middleware =
  filterPathMiddleware; // ./js/src/block-editor.js
...
wp.apiFetch.use(function (options, next) {
  ...
  return next(
    filter_path_middleware(options, pllFilteredRoutes, addLanguageParameter),
  );
});
...
function addLanguageParameter(options) {
  if ("undefined" === typeof options.data || null === options.data) {
    // GET
    options.path +=
      (options.path.indexOf("?") >= 0 ? "&lang=" : "?lang=") +
      getCurrentLanguage();
  } else {
    // PUT, POST
...
jQuery(function ($) {
  ...
  $(".post_lang_choice").on("change", function (event) {
    ...
    function isEmptyPost() {
      ...
      return (
        !editor.getEditedPostAttribute("title")?.trim() &&
        !editor.getEditedPostContent() &&
        !editor.getEditedPostAttribute("excerpt")?.trim()
      );
    }
